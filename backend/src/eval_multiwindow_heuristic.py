"""
Phase 11a: production-faithful eval.

Phase 10b used expert tonic on both sides of the eval (templates AND
queries), which inflates the deployment estimate because real users
don't have .tonicFine annotations. This script re-runs the multi-window
query eval but with the runtime heuristic tonic detector on the query
side. The templates still use expert tonic (those are built offline from
the CompMusic .tonicFine files, which we'll always have at template-
build time).

This number is what production will actually serve once we ship.

Setup:
  Templates: 5-min pyin + EXPERT tonic (reuses X_tdms_long.npy from Phase 10a).
  Queries:   Three 60s windows averaged, pyin + HEURISTIC tonic (predict.py._detect_tonic).
  Classifier: 1-NN with symmetric KL.
  Eval: 5-fold stratified CV, 5 seeds, recording-aware.

Outputs:
  data/X_tdms_audio_multiwindow_heuristic.npy
  data/eval_multiwindow_heuristic_report.txt
"""
...
def extract_one(args: tuple) -> dict:
    """Three 60s windows, pyin + heuristic tonic per window, average TDMSs."""
...
def main() -> None:
    ...
    if args.skip_extract and os.path.exists("data/X_tdms_audio_multiwindow_heuristic.npy"):
        X_query = np.load("data/X_tdms_audio_multiwindow_heuristic.npy")
    else:
        jobs = [(i, m["recording_id"], int(y_full[i])) for i, m in enumerate(meta)]
        print(f"Extracting 3x60s multi-window queries with HEURISTIC tonic, {args.workers} workers")
        ...
        np.save("data/X_tdms_audio_multiwindow_heuristic.npy", X_query)
    ...
    lines = [
        "=" * 78,
        "Phase 11a: production-faithful eval",
        f"Templates: 5-min pyin + EXPERT tonic     Queries: 3 x 60s avg, pyin + HEURISTIC tonic",
        f"Recordings: {int(keep.sum())}/480",
        "=" * 78,
        "",
        f"  Phase 11a (heuristic tonic on queries)  top1 {m[0]*100:5.2f}% +/- {s[0]*100:.2f}   top5 {m[1]*100:5.2f}% +/- {s[1]*100:.2f}",
        "",
        "Comparison ladder:",
        "  Phase 10b  pyin + EXPERT    tonic, 5min + 3x60s avg   top1 73.23%   top5 95.31%",
        f"  Phase 11a  pyin + heuristic tonic, 5min + 3x60s avg   top1 {m[0]*100:5.2f}%   top5 {m[1]*100:5.2f}%",
        "  Phase 8    pyin + heuristic tonic, 60s + 60s            top1 37.36%   top5 67.91%",
        "  v1 deployed                                              top1  8.32%   top5 28.57%",
        "",
        "Phase 11a is the number production will actually serve.",
    ]
    write_report("data/eval_multiwindow_heuristic_report.txt", lines)
    print("\n" + "\n".join(lines))
