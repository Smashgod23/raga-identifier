"""
Does adding the YouTube recordings to DeepSRGM TRAINING lift novel-audio accuracy?
This is the test of whether the existing data was actually exhausted.

Fair, leakage-free comparison. Recording-aware split the YouTube recordings into
YT-train / YT-test. Then evaluate the SAME held-out YT-test under two models:
  A (baseline):  train on CompMusic-only (480 clean studio recordings)
  B (augmented): train on CompMusic + YT-train
If B beats A on YT-test, real concert audio in training closes the OOD gap and the
data was NOT exhausted.

Requires data/deepsrgm_essentia_seqs.npz (CMD) and data/deepsrgm_youtube_seqs.npz
(YouTube, from build_youtube_contours.py).
"""
import json
import os
import sys
...
def main():
    ...
    print(f"CMD {len(cmd_s)} recs, YouTube {len(yt_s)} recs, device={dev}", flush=True)
    ...
    def train(seqs, labels, train_ids, rng):
        Xtr, ytr = subseqs(seqs, train_ids, labels, PER_REC, rng)
    ...
    def evalrecs(net, seqs, labels, test_ids, rng):
        net.eval(); t1 = t5 = 0
        with torch.no_grad():
            for ri in test_ids:
                Xte, _ = subseqs(seqs, [ri], labels, 20, rng)
    ...
    A1s, A5s, B1s, B5s = [], [], [], []
    cmd_all = list(range(len(cmd_s)))
    for seed in SEEDS:
        torch.manual_seed(seed); rng = np.random.default_rng(seed)
        # recording-aware 70/30 split of YouTube per raga
        by_cls = {}
        ...
        # Arm A: CMD-only -> eval YT-test
        netA = train(cmd_s, cmd_y, cmd_all, rng)
        a1, a5 = evalrecs(netA, yt_s, yt_y, yt_test, rng)
        # Arm B: CMD + YT-train -> eval YT-test (concatenate the two recording pools)
        comb_s = cmd_s + [yt_s[i] for i in yt_train]
        comb_y = np.concatenate([cmd_y, yt_y[yt_train]])
        netB = train(comb_s, comb_y, list(range(len(comb_s))), rng)
        b1, b5 = evalrecs(netB, yt_s, yt_y, yt_test, rng)
        A1s.append(a1); A5s.append(a5); B1s.append(b1); B5s.append(b5)
        print(f"  [seed {seed}] n_test={len(yt_test)}  A(CMD-only) {a1:.1f}/{a5:.1f}  B(CMD+YT) {b1:.1f}/{b5:.1f}", flush=True)

    print("\n=== YouTube-in-training test (held-out YT-test, recording-aware) ===")
    print(f"  A  CMD-only:    top-1 {np.mean(A1s):5.1f} +/- {np.std(A1s):.1f}   top-5 {np.mean(A5s):5.1f}")
    print(f"  B  CMD+YouTube: top-1 {np.mean(B1s):5.1f} +/- {np.std(B1s):.1f}   top-5 {np.mean(B5s):5.1f}")
