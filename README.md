...
Later I came back to this with one more idea: split the work so the server only resolves the audio URL (the cheap step that was not getting blocked) and the user's own browser downloads the bytes from their residential IP, which YouTube does not throttle. I tested it directly. The `android_vr` and `tv_embedded` clients do hand back direct googlevideo.com URLs that honor range requests, so I could pull the first 100 KB with a normal HTTP request. The problem is that those responses carry no `Access-Control-Allow-Origin` header, and the CORS preflight returns 400, so a browser would refuse to let my JavaScript read the bytes even though the request itself succeeds. The web player's own stream URLs would not help either, because any CORS header there is scoped to youtube.com, not to my site. That kills the clean "server resolves, browser downloads" architecture for good. The one browser capability that gets around it is `getDisplayMedia`, which captures the audio the tab is already playing rather than fetching anything, so it touches neither the blocked IP nor a CORS check. That path works but is Chrome and Edge only and needs a share-tab permission and a real-time capture window, so it is logged as a candidate rather than shipped.
...
## The Road to a Model That Actually Works (Phases 6-13)

This is the part of the project I am most proud of, and it is also the part where I was wrong the most times. I am writing it as a continuous story because the wrong turns are the point. If you only read the final number you miss everything I learned. The phase-by-phase sections below have the exact numbers and code; this is the human version of how I got there.

It started with a gut check. I had shipped v1 at "84.4%" and moved on, but something nagged at me, so I rebuilt the evaluation from scratch with a proper recording-aware split. The 84.4% evaporated. On a fair split v1 was 71.79%, and worse, when I measured what the *deployed* app actually does to a real uploaded file (run pyin, guess the tonic, build the feature) it scored 8.32% on its own benchmark. The number I had been proud of was a measurement artifact. The live site was barely above random and I had not known. That stung, but it reset the whole project on honest footing, and every number after this point is measured through the exact path a real upload takes.

The reason allied ragas were getting confused was that my feature, a pitch histogram, throws away the order notes are played in. Two ragas with the same notes but different phrasing look identical to it. I found the paper that solved exactly this (Gulati et al., the Time-Delayed Melody Surface) and reimplemented it. It matched their published 86.7% on my data to two decimals. For about a day I thought I was done.

Then I tested it the way a user would, on raw audio instead of the dataset's hand-cleaned pitch files, and it collapsed to 37%. The expert features the paper used are not something a phone recording comes with. This was the real problem the whole rest of the journey is about: the gap between research numbers (computed on pristine, human-annotated pitch and tonic) and what you can actually extract from an uploaded file in real time.

I spent a long stretch grinding at that gap, and a lot of it was dead ends. I had a re-ranker model for tonic detection that looked good in isolation; wired into the real pipeline it made things slightly worse, because it had been trained on a different slice of each recording than production uses. I convinced myself the fix was a "bidirectional octave fold" and wrote it into this very README as the plan, then ran the experiment and proved my own claim was wrong: octave errors do not even matter, because the feature is octave-folded. I had to delete my own confident plan.

The thing that did work in that stretch came from music, not machine learning: in Carnatic music a tanpura drone plays the tonic Sa and its fifth Pa continuously underneath everything. So the true tonic is the one candidate that has strong energy at both of those positions. Scoring on that drone signature instead of generic "peakiness" jumped tonic detection from 52% to 70% with no model at all. Stacking longer reference templates and averaging several windows of the query pushed the deployable raga accuracy up to the low 40s. Real progress, but still nowhere near where I wanted it.

Twice in here I got excited and quoted a number that was too good, and twice the person I was building this for caught it and made me walk it back. Once I called a 73% figure "deployable" when it had quietly used the dataset's expert tonic; once I estimated "about 70%" for a short upload when the real measured number was closer to 55%. Both corrections were right. The discipline I took from it: never call a number deployable unless it came through the exact runtime path, and measure the short-clip case instead of guessing it.

The breakthrough was a refusal to accept a ceiling. I had concluded, and stated, that 75% was simply unreachable on real audio because my pitch extractor (pyin) caps out around 73% even with a perfect tonic. I was told, bluntly, to find another way. So I stopped reimplementing pitch and tonic detection by hand and asked a simpler question: what did the people who built the dataset actually use? The answer was two specific algorithms (Melodia for pitch, a multipitch method for tonic), and both of them ship in a library called Essentia, and there was never any reason they could only run offline. I ran the real expert algorithms on raw audio at inference. Tonic detection went to 85%. The full pipeline went to 75.62% top-1, 91% top-5, nine times what the live site had actually been doing.

The honest ending: that 75% needs a few minutes of audio to reach (a 30-second clip is 34%, two minutes is 64%, three minutes is 69%), and I have not yet verified it on the live server because I happened to be on a hotel network that blocks GitHub when I finished. But the path from an 8% app to a 75% one is real, it is measured, and most of it came from being wrong in public and fixing it.
...
I kept the caveats honest, so I measured the two that mattered instead of guessing. The 89 percent is on expert pitch tracks, but inference uses Essentia pitch, so I extracted Essentia Melodia contours for all 480 recordings and retrained on those. In-distribution that gives 77.9 percent plus or minus 2.9 top-1 and 91.2 percent top-5. The drop from 89 to 78 is the price of Essentia pitch being noisier than the hand-corrected expert tracks, and it tells me that pitch-extraction quality is itself worth about eleven points, which makes a better pitch tracker a real lever rather than a guess.

Then I measured the number that actually matters: top-1 on novel audio. I trained the deployable model on all 480 CompMusic recordings and classified YouTube recordings it had never seen. It scored 58.4 percent top-1 and 80.5 percent top-5. That is the honest real-world number, and it puts the whole ladder in order: on novel audio, v1 gets about 17 percent, TDMS about 50 percent, and the sequence model 58.4 percent, with top-5 going 43 to 70 to 80.5 percent across the three. The sequence model is the best thing the project has, and its top-5 reaches 80 percent on real recordings, which is genuinely useful because the app shows five candidates. The YouTube labels come from a search query, so a few of the misses are probably mislabeled videos rather than model errors, which means the true top-1 is likely a little higher than 58.

So I have not hit 80 percent top-1 on novel audio, and I want to be plain about that. But I went from a model that scored 17 percent on real audio to one that scores 58, I proved exactly why (allied ragas need phrase order, not just a distribution), and I have a measured, deployable model that beats everything before it.
...
One disclosure, because the review of this commit caught it and it deserves to be in the record: the window length itself (1600 over 800 tokens) was picked by top-1 on the same 194 recordings before the cross-validation ran. So the tonic-correction gain of +3.6 is honestly cross-validated, but the absolute 66.0 and 69.6 levels inherit one in-sample choice. Folding window length into the CV config space is on the list; I expect the effect to be small since all three lengths scored within 1.6 points, but expecting is not measuring, and this section exists precisely because those two things keep turning out different.

## Phase 17: Teaching the Model to Judge Its Own Tonic Frame, and Finding the Plateau

The margin gate in Phase 16 was a dumb selector bolted onto a smart model, and the oracle gap said a smarter selector had about thirteen points to harvest. So I built the smart version: the network itself learns to judge whether a contour is in the correct tonic frame. The model got a second output head trained on deliberately sabotaged examples, subsequences shifted by exactly the errors the tonic detector makes (a fifth, a fourth, an octave, folded back into range). The raga loss only ever sees correct frames, because teaching the classifier that a shifted Kalyani is still Kalyani would create transposition invariance and destroy the allied-raga discrimination the whole sequence model exists for. While I was retraining anyway, I fixed a flaw the review had flagged: padding was encoded as a real pitch value, and the attention could stare at it. Padding is now a dedicated token the attention is masked away from. And window length went inside the cross-validation this time, paying off the disclosure from Phase 16.

I also tightened the test so the new method had to earn it. The config space for the cross-validation contained doing nothing, the old confidence-margin selector, and the new learned selector, so the learned head had to beat both alternatives on folds it had not been selected on.

It did, in all five folds. And the result taught me something I did not expect: the learned selector is genuinely better, but its advantage lands almost entirely in top-5, not top-1. The pooled held-out score is 69.6 top-1 and 87.1 top-5, against 69.6 and 85.6 for Phase 16's margin gate. Same top-1, to the decimal. The attribution check confirmed the architecture changes were clean: without any tonic correction, v3 matches v2's top-1 exactly and gains a point of top-5 from the padding fix.

That flat top-1 is the finding. Three architecturally different approaches now converge at 69.6: a confidence margin, a learned frame judge, and everything in between. The oracle headroom shrank to about ten points, the fold-to-fold spread is nearly fifteen points, and differences under two points are not resolvable with 194 test recordings. I take the convergence seriously: this is the plateau of the current model family on this data with these labels. And the labels matter, because the evaluation set is auto-labeled from search results, so some unknown slice of the remaining 30 percent is not model error at all, it is recordings that are not what their label says. The measured 69.6 is a floor on the true accuracy.

So the scorecard after three phases of accuracy work: novel-audio top-1 went 17, 50, 64, 66, 69.6, and top-5 went 43, 70, 82, 85.6, 87.1. Top-5 is comfortably past the 80 goal. Top-1 is not, and getting it past this plateau is no longer a matter of another clever inference trick. The honest paths remaining are structural: expert-verified labels for both evaluation and training, more genuinely clean recordings, or representation changes like phrase-gap tokens and finer pitch bins that each need a re-extraction and carry no guarantee. Nothing is deployed yet. The live site still runs v1, and the bar for changing that has not moved.
